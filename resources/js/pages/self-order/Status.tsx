                ...
                <div className="mx-auto max-w-lg pt-8 text-center">
                    <div className="mb-5 flex justify-center">
                        {isCancelled ? (
                            ...
                            <div className="flex h-24 w-24 items-center justify-center rounded-full bg-gradient-to-br from-[#6E8C8A] to-[#4F6B6A] shadow-lg shadow-[#4F6B6A]/25 animate-pulse-dot">
                                    ...
                                    <div
                                        className={cn(
                                            'flex h-10 w-10 shrink-0 items-center justify-center rounded-full border-2 transition-all duration-500',
                                                ...
                                                : active
                                                    ? 'border-[#4F6B6A] bg-[#4F6B6A]/12 text-[#4F6B6A] animate-pulse-dot'
                                        ...
                                        <div
                                            className={cn(
                                                ...
                                                i < current
                                                    ? 'bg-[#4F6B6A]/60'
                                                    : i === current
                                                        ? 'bg-gradient-to-b from-[#4F6B6A] to-gray-200 animate-progress-glow'
                                                        : 'bg-gray-200',
                                            )}
                                        />
                                    )}
                                    ...
                                    {item.options?.length > 0 && (
                                        <p className="mt-1 text-xs text-gray-500">
                                            {item.options
                                                ...
                                                .map(o => o.quantity > 1 ? `${o.option_item.name} x${o.quantity}` : o.option_item.name)
                                                .join(', ')}
                                        </p>
                                    )}
                                    {item.notes && (
                                        <p className="mt-0.5 text-xs text-gray-400 italic">Catatan: {item.notes}</p>
                                    )}
                                </div>
                            </div>
                        ))}
