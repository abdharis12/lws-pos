...
export default function ShiftsIndex({
    ...
    function prevWeek() {
        ...
        router.get(
            '/admin/shifts',
            { week_start: prev.toISOString().slice(0, 10) },
    ...
    function nextWeek() {
        ...
        router.get(
            '/admin/shifts',
            { week_start: next.toISOString().slice(0, 10) },
            { preserveScroll: true, preserveState: false },
        );
    }
            ...
            <div className="mb-8 flex flex-col justify-between gap-4 border-b border-[#CFC0A4]/40 pb-6 sm:flex-row sm:items-end">
                <div>
                    <div className="flex items-center gap-2 text-xs font-semibold tracking-[0.2em] text-[#CFC0A4] uppercase">
                        <Calendar className="size-3.5 text-[#4F6B6A]" />
            ...
            <div className="flex flex-col gap-6">
                ...
                <div className="flex items-center justify-between">
                    <Button
                        ...
                        size="sm"
                        onClick={prevWeek}
                        className="gap-1 border-[#CFC0A4]/40 text-[#4F6B6A] hover:bg-[#CFC0A4]/10"
                    >
                        <ChevronLeft className="size-4" />
                        Minggu Sebelumnya
                    </Button>
                    <span className="font-serif text-sm font-semibold text-[#4F6B6A]">
                        {formatDateShort(dates[0]).dayName},{' '}
                        {formatDateShort(dates[0]).dayNum}{' '}
                        {formatDateShort(dates[0]).month}{' '}
                        {formatDateShort(dates[0]).year} —{' '}
                        {formatDateShort(dates[6]).dayName},{' '}
                        {formatDateShort(dates[6]).dayNum}{' '}
                        {formatDateShort(dates[6]).month}{' '}
                        {formatDateShort(dates[6]).year}
                    </span>
                    <Button
                        variant="outline"
                        size="sm"
                        ...
                        <div className="flex flex-wrap items-end gap-4">
                                ...
                                >
                                    {dates.map((d) => (
                                        <option key={d} value={d}>
                                            {formatDateShort(d).dayName},{' '}
                                            {formatDateShort(d).dayNum}{' '}
                                            {formatDateShort(d).month}{' '}
                                            {formatDateShort(d).year}
                                        </option>
                                    ))}
                                </select>
                            </div>
                            ...
                            <div className="grid gap-2">
                                ...
                                <select
                                    ...
                                    onChange={(e) =>
                                        setData('shift_number', e.target.value)
                                    }
                                >
                            ...
                            <Button
                                ...
                                className="gap-2 bg-[#4F6B6A] font-serif text-white hover:bg-[#3B5655]"
                            >
                            ...
                            return (
                                <div
                                    ...
                                    className={`rounded-xl border p-3 transition-all ${
                                        isToday
                                            ? 'border-[#CFC0A4]/60 bg-[#CFC0A4]/5 shadow-sm'
                                            : 'border-[#CFC0A4]/20 bg-white/60'
                                    }`}
                                >
                                    <button
                                        type="button"
                                    ...
                                    >
                                        ...
                                        <p
                                            className={`font-serif text-lg font-bold ${isToday ? 'text-[#CFC0A4]' : 'text-slate-800'}`}
                                        >
                                            {dayNum}
                                        </p>
                                        <span
                                    ...
                                    <div className="space-y-1.5">
                                        {dayShifts.map((shift) => (
                                            <div
                                                key={shift.id}
                                                className="group relative cursor-pointer rounded-lg bg-[#4F6B6A]/10 p-2 text-xs transition-colors hover:bg-[#4F6B6A]/20"
                                                onClick={() =>
                                                    handleEditShift(shift)
                                                }
                                            >
                                                <p className="truncate font-medium text-[#4F6B6A]">
                                                    {shift.employee.user.name}
                                                </p>
                                                <p className="text-slate-500">
                                                    S{shift.shift_number}{' '}
                ...
                <div className="overflow-x-auto">
                    <div className="rounded-xl border border-[#CFC0A4]/20 bg-white/60">
                        <table className="w-full text-left text-xs">
                            <thead>
                                <tr className="bg-[#4F6B6A]/5">
                                    <th className="sticky left-0 z-10 min-w-[160px] bg-[#4F6B6A]/5 px-3 py-2.5 font-semibold text-[#4F6B6A]">
                                        Karyawan
                                    </th>
                                    {dates.map((date) => {
                                        const { dayName, dayNum, month, year } =
                                        ...
                                        return (
                                            ...
                                            >
                                                ...
                                                <p
                                                    className={`font-serif text-base ${isToday ? 'font-bold' : ''}`}
                                                >
                                                    {dayNum}
                                                </p>
                            ...
                            <tbody className="divide-y divide-[#CFC0A4]/10">
                                {employees.length === 0 && (
                                    <tr>
                                        <td
                                ...
                                {employees.map((emp) => {
                                    ...
                                    return (
                                        <tr
                                            key={emp.id}
                                            className="transition-colors hover:bg-[#4F6B6A]/[0.02]"
                                        >
                                            ...
                                            {empShifts.map((shift, i) => (
                                                ...
                                                >
                                                    {shift ? (
                                                        <div className="inline-flex items-center gap-1 rounded-md bg-[#4F6B6A]/8 px-2 py-1.5">
                                                            <span className="font-medium whitespace-nowrap text-[#4F6B6A]">
                                                                S
                                                                {
                                                                    shift.shift_number
                                                                }{' '}
                                                            ...
                                                            <button
                                                                ...
                                                                className="ml-0.5 rounded p-0.5 text-slate-400 transition-colors hover:bg-[#4F6B6A]/10 hover:text-[#4F6B6A]"
                                                                title="Edit shift"
                                                            >
                ...
                <Card className="border-[#CFC0A4]/40 bg-white shadow-sm backdrop-blur-sm">
                    <CardHeader
                        className="cursor-pointer border-b border-[#CFC0A4]/20 transition-colors hover:bg-[#CFC0A4]/5"
                        onClick={() => setShowMonthly(!showMonthly)}
                    >
                        <div className="flex items-center justify-between">
                            <div className="flex items-center gap-2">
                                <ClipboardList className="size-4 text-[#4F6B6A]" />
                            ...
                            <div className="flex items-center gap-4">
                                <span className="text-xs text-slate-500">
                                    {monthlyGrandTotal} shift dari{' '}
                                    {activeEmployeeCount} karyawan aktif
                                </span>
                                <ChevronRight
                                    className={`size-4 text-slate-400 transition-transform ${showMonthly ? 'rotate-90' : ''}`}
                                />
                            </div>
                        </div>
                    </CardHeader>
                    {showMonthly && (
                        <CardContent className="pt-5">
                            <div className="grid gap-6 lg:grid-cols-2">
                                {/* Per Day Table */}
                                <div>
                                    <h4 className="mb-3 font-serif text-sm font-semibold text-[#4F6B6A]">
                                        Per Hari
                                    </h4>
                                    <div className="overflow-x-auto rounded-lg border border-[#CFC0A4]/20">
                                        <table className="w-full text-left text-xs">
                                            <thead>
                                                <tr className="bg-[#4F6B6A]/5">
                                                    <th className="px-3 py-2 font-semibold text-[#4F6B6A]">
                                                        Tanggal
                                                    </th>
                                                    <th className="px-3 py-2 text-right font-semibold text-[#4F6B6A]">
                                                        Jumlah Shift
                                                    </th>
                                                </tr>
                                            </thead>
                                            ...
                                            <tfoot className="bg-[#4F6B6A]/5">
                                                <tr>
                                                    <td className="px-3 py-2 font-semibold text-[#4F6B6A]">
                                                        Total
                                                    </td>
                                                    <td className="px-3 py-2 text-right font-semibold text-[#4F6B6A]">
                                                        {monthlyGrandTotal}
                                                    </td>
                                                </tr>
                                            </tfoot>
                ...
                {selectedDayShifts && (
                    ...
                    >
                        <div
                            className="mx-4 w-full max-w-md rounded-2xl border border-[#CFC0A4]/40 bg-[#F6F2E9] p-6 shadow-xl"
                            onClick={(e) => e.stopPropagation()}
                        >
                            <h3 className="mb-1 font-serif text-lg font-semibold text-[#4F6B6A]">
                                Daftar Shift
                            </h3>
                            <p className="mb-4 text-sm text-slate-500">
                                {formatDateLong(selectedDayShifts.date)}
                            </p>
                            {selectedDayShifts.shifts.length === 0 ? (
                                ...
                                <div className="space-y-2">
                                    {selectedDayShifts.shifts.map((shift) => (
                                        <div
                                            key={shift.id}
                                            className="flex items-center justify-between rounded-lg border border-[#CFC0A4]/20 bg-white px-3 py-2.5 text-sm"
                                        >
                                            <div>
                                                <p className="font-medium text-slate-800">
                                                    {shift.employee.user.name}
                                            ...
                                            <div className="flex items-center gap-2">
                                                <span className="rounded-md bg-[#4F6B6A]/10 px-2.5 py-1 font-medium whitespace-nowrap text-[#4F6B6A]">
                                                    S{shift.shift_number}{' '}
                                                    {shift.start_time.slice(
                                                        0,
                                                        5,
                                                ...
                                                <button
                                                    ...
                                                    className="rounded p-1 text-slate-400 transition-colors hover:bg-[#4F6B6A]/10 hover:text-[#4F6B6A]"
                                                    title="Edit shift"
                                                >
                ...
                {editingShift && (
                    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/30">
                        <div className="w-full max-w-sm rounded-2xl border border-[#CFC0A4]/40 bg-[#F6F2E9] p-6 shadow-xl">
                            <h3 className="mb-4 font-serif text-lg font-semibold text-[#4F6B6A]">
                                Edit Shift
                            </h3>
                            <div className="space-y-3">
                                <div className="rounded-lg border border-[#CFC0A4]/20 bg-[#4F6B6A]/[0.02] p-3 text-sm">
                                    <div className="grid grid-cols-[auto_1fr] gap-x-3 gap-y-1.5">
                                        <span className="text-slate-400">
                                            Nama Karyawan
                                        </span>
                                        <span className="font-medium text-slate-800">
                                            {editingShift.employee.user.name}
                                        </span>
                                        <span className="text-slate-400">
                                            Posisi
                                        </span>
                                        <span className="text-slate-600">
                                            {editingShift.employee.position}
                                        </span>
                                        <span className="text-slate-400">
                                            Tanggal
                                        </span>
                                ...
                                <div className="grid gap-2">
                                    ...
                                    <input
                                        type="time"
                                        className="flex h-9 w-full rounded-md border border-[#CFC0A4]/50 bg-white px-3 py-1 text-sm shadow-xs focus:border-[#4F6B6A] focus:ring-[#4F6B6A]"
                                        value={data.start_time}
                                        onChange={(e) =>
                                            setData(
                                                'start_time',
                                ...
                                <div className="flex gap-3">
                                    <Button
                                        ...
                                        disabled={processing}
                                        className="flex-1 bg-[#4F6B6A] font-serif text-white hover:bg-[#3B5655]"
                                    >
                                        Simpan
                                    </Button>
                                    <Button
                                        ...
                                        className="flex-1 border-[#CFC0A4]/40 text-slate-600 hover:bg-[#CFC0A4]/10"
                                    >
                                        Batal
                                    </Button>
                                </div>
                        ...
                        <DialogHeader>
                            ...
                            <DialogDescription className="text-center text-slate-500">
                                ...
                                <span className="font-semibold text-[#4F6B6A]">
                                    {shiftToDelete?.employee.user.name}
                                </span>{' '}
                                pada{' '}
                                <span className="font-semibold text-[#4F6B6A]">
                                    {formatDateLong(
