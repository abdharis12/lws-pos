  ...
  <div className="relative z-30 max-w-7xl w-full mx-auto px-4 sm:px-6 lg:px-8 pt-4 sm:pt-6 flex items-center justify-between">
    <div className="flex items-center gap-2 text-xs font-sans text-[#4F6B6A]">
      ...
      <span className="font-semibold uppercase tracking-widest text-[11px]">
        Maison de LW's Senopati • Carte du Jour
      </span>
    </div>
...
const BadgeRow = ({ dish }: { dish: DishData }) => (
  <div className="flex items-center gap-2.5 flex-wrap">
    <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-sans font-semibold tracking-wide bg-[#4F6B6A]/15 text-[#233433] border border-[#4F6B6A]/30 shadow-xs">
      ...
      <span>Menu Spesial Hari Ini: {dish.label}</span>
    </span>
    ...
    <AnimatePresence mode="wait">
      ...
      >
        ...
        <div className="pt-2 flex items-baseline gap-3">
          ...
          <span className="text-xs font-sans text-[#4F6B6A]/75">Nett Termasuk Pajak</span>
        </div>
      ...
      {dishes.map((dish, idx) => {
        ...
        return (
          <button
            ...
            className={`flex items-center gap-2 sm:gap-2.5 px-2.5 sm:px-3 py-1.5 rounded-xl transition-all duration-300 text-left focus:outline-none cursor-pointer ${
              isSelected ? 'bg-[#4F6B6A]/10 border border-[#4F6B6A]/25 shadow-xs' : 'hover:bg-black/5 border border-transparent'
            }`}
          >
            <div
              className={`relative w-8 h-8 sm:w-10 sm:h-10 rounded-full overflow-hidden transition-all duration-300 ${
                isSelected
            ...
            <div className="flex flex-col">
              <span
                className={`font-sans text-xs sm:text-sm font-semibold transition-colors ${
                  isSelected ? 'text-[#233433]' : 'text-[#4F6B6A]/75'
                }`}
              >
                {dish.label}
              </span>
